...
export default function Home() {
  ...
  return <main className="page">
    ...
    <section className="poster" id="top">
      <nav className="nav"><a href="#top">Барбер и Барби</a><div className="nav-links"><a href="#services">Услуги</a><a href="#about">О заведении</a><a href="#reviews">Отзывы</a><a href="#booking">Запись</a></div></nav>
    ...
    <section className="section" id="services"><div className="shell"><div className="section-head"><h2>Услуги</h2></div><div className="grid services">{services.map(service => <article className="service-card" key={service.title}><h3>{service.title}</h3><ul className="service-list">{service.items.map(([title, description, price]) => <li className="service-item" key={title}><div><strong>{title}</strong><p>{description}</p></div><span className="service-price">{price}</span></li>)}</ul></article>)}</div></div></section>

    <section className="section section-dark" id="about"><div className="shell local-copy"><div className="section-head"><h2>О заведении</h2></div><p>Опытный барбер, специализирующийся на мужских стрижках, фейде, оформлении бороды и бритье опасной бритвой. Индивидуальный подход, качественный уход, точность, скорость и безупречный результат.</p><p>Просторное и чистое пространство с современным ремонтом, где комфортно провести время и расслабиться во время стрижки.</p></div></section>
